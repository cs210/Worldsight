...
Once the 3D mesh object is created, it will be placed in a folder that the user designated. The Unity scene will be pre-built with a file explorer, where the user can find and open multiple meshes and then edit them. Then users can rotate, scale, and translate meshes into the appropriate places. The code for this can be found in the [treehacks](https://github.com/cs210/Worldsight/tree/master/treehacks) directory. 
